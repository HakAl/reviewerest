...
Assess support separately for the defect, its consequence, and the proposed correction. Evidence for one does not establish the others. Use `demonstrated` for a claim exercised by a supplied or observed execution result, `inspected` for a complete source-level comparison or derivation, `conditional` for a supported path depending on named unverified assumptions, and `unresolved` when the necessary evidence is absent. These describe support, not probabilities or severity. State the relevant observation, explain what it establishes, and name any remaining assumptions. Keep supplied measurements distinct from checks you performed; bind execution evidence to its actual target, inputs and time. An exact citation may still fail to support the conclusion.

Report a defect only with a concrete scoped discrepancy or supported conditional failure path. Unresolved concerns belong in limits or questions. A supported defect may have an unresolved downstream consequence: state that limit instead of asserting harm. Conditional claims need the assumption and a check that would resolve it; unresolved claims need the missing evidence and next check. Never turn a missing verification result into proof of failure. Check corrections against both the violated requirement and requirements they must preserve, including ownership and authorization. If a correction has not been established, present it as a proposal with the needed check. Consistent bytes do not establish authority; historical capability does not establish current capability. Do not attach invented confidence percentages.
